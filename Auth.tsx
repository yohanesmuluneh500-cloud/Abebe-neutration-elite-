    ...
    try {
      if (isSignUp) {
        ...
        if (authData.user) {
          ...
          if (!authData.session) {
            setError("Warrior Registered. Login with your Name and Password.");
            setIsSignUp(false);
          }
        }
      } else {
    ...
    <div className="min-h-screen bg-[#050505] flex items-center justify-center p-4">
      <div className="max-w-md w-full glass p-8 rounded-2xl border border-zinc-800 shadow-2xl animate-in zoom-in duration-500">
        <div className="text-center mb-8">
          ...
          <p className="text-blue-300/40 text-[10px] tracking-[0.3em] uppercase font-bold">Secure Entry Terminal</p>
        </div>
        ...
        <form onSubmit={handleAuth} className="space-y-5">
          <div className="space-y-2">
            ...
            <input 
              ...
              className="w-full bg-zinc-900 border border-zinc-800 rounded-lg p-3.5 text-white focus:border-blue-600 outline-none transition-all text-lg font-bold"
          ...
          {isSignUp && (
            <div className="grid grid-cols-3 gap-3 animate-in slide-in-from-top duration-300">
              <div className="space-y-2">
                ...
                <input required type="number" value={age} onChange={(e) => setAge(e.target.value)} className="w-full bg-zinc-900 border border-zinc-800 rounded-lg p-3 text-white focus:border-blue-600 outline-none" />
              </div>
              <div className="space-y-2">
                <label className="text-[10px] font-bold text-zinc-500 block uppercase tracking-widest">Height (cm)</label>
                <input required type="number" value={height} onChange={(e) => setHeight(e.target.value)} className="w-full bg-zinc-900 border border-zinc-800 rounded-lg p-3 text-white focus:border-blue-600 outline-none" />
              </div>
              <div className="space-y-2">
                <label className="text-[10px] font-bold text-zinc-500 block uppercase tracking-widest">Weight (kg)</label>
                <input required type="number" value={weight} onChange={(e) => setWeight(e.target.value)} className="w-full bg-zinc-900 border border-zinc-800 rounded-lg p-3 text-white focus:border-blue-600 outline-none" />
              </div>
            </div>
          )}
          ...
          {error && <div className="p-3 bg-red-900/10 border border-red-900/30 text-red-500 text-[10px] font-bold rounded-lg tracking-wider uppercase">{error}</div>}
